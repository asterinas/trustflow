# This file is a 'toolchain description file' for CMake. It teaches CMake about
# the Emscripten compiler, so that CMake can generate makefiles from
# CMakeLists.txt that invoke emcc.

# To use this toolchain file with CMake, invoke CMake with the following command
# line parameters: cmake
# -DCMAKE_TOOLCHAIN_FILE=<EmscriptenRoot>/cmake/Modules/Platform/Emscripten.cmake
# -DCMAKE_BUILD_TYPE=<Debug|RelWithDebInfo|Release|MinSizeRel> -G "Unix
# Makefiles" (Linux and OSX) -G "MinGW Makefiles" (Windows)
# <path/to/CMakeLists.txt> # Note, pass in here ONLY the path to the file, not
# the filename 'CMakeLists.txt' itself.
...
# OpenCV) use this to detect bitness. Allow users to ovewrite this on the
# command line with -DEMSCRIPTEN_SYSTEM_PROCESSOR=arm.
...
# Locate where the Emscripten compiler resides in relative to this toolchain
# file.
if("${EMSCRIPTEN_ROOT_PATH}" STREQUAL "")
  get_filename_component(GUESS_EMSCRIPTEN_ROOT_PATH
...
# If not found by above search, locate using the EMSCRIPTEN environment
# variable.
if("${EMSCRIPTEN_ROOT_PATH}" STREQUAL "")
...
# try to find emcc
if("${EMSCRIPTEN_ROOT_PATH}" STREQUAL "")
  execute_process(COMMAND which emcc OUTPUT_VARIABLE EMSCRIPTEN_EMCC_PATH)
  set(EMSCRIPTEN_ROOT_PATH "${EMSCRIPTEN_EMCC_PATH}/..")
endif()
...
message("-- find EMSCRIPTEN_ROOT_PATH: ${EMSCRIPTEN_ROOT_PATH}")
...
if("${CMAKE_C_COMPILER}" STREQUAL "")
  set(CMAKE_C_COMPILER
      "${EMSCRIPTEN_ROOT_PATH}/emcc${EMCC_SUFFIX}"
      CACHE FILEPATH "Emscripten emcc")
endif()
if("${CMAKE_CXX_COMPILER}" STREQUAL "")
  set(CMAKE_CXX_COMPILER
      "${EMSCRIPTEN_ROOT_PATH}/em++${EMCC_SUFFIX}"
      CACHE FILEPATH "Emscripten em++")
endif()

if("${CMAKE_AR}" STREQUAL "")
  set(CMAKE_AR
      "${EMSCRIPTEN_ROOT_PATH}/emar${EMCC_SUFFIX}"
      CACHE FILEPATH "Emscripten ar")
endif()

if("${CMAKE_RANLIB}" STREQUAL "")
  set(CMAKE_RANLIB
      "${EMSCRIPTEN_ROOT_PATH}/emranlib${EMCC_SUFFIX}"
      CACHE FILEPATH "Emscripten ranlib")
endif()
...
# Emscripten. Pass -DEMSCRIPTEN_FORCE_COMPILERS=OFF to disable (sensible mostly
...
if(EMSCRIPTEN_FORCE_COMPILERS)
  ...
  # CMake understands Clang better. Toolchain script is interpreted multiple
  # times, so don't rerun the check if already done before.
  if(NOT CMAKE_C_COMPILER_VERSION)
    ...
    string(REGEX MATCH "clang version ([0-9\\.]+)" _dummy_unused
  ...
  if(NOT EMSCRIPTEN_VERSION)
    ...
    if(NOT _cmake_compiler_result EQUAL 0)
      message(
        FATAL_ERROR
          "Failed to fetch Emscripten version information with command \"'${CMAKE_C_COMPILER}' -v\"! Process returned with error code ${_cmake_compiler_result}."
      )
    endif()
    string(REGEX MATCH "emcc \\(.*\\) ([0-9\\.]+)" _dummy_unused
                 "${_cmake_compiler_output}")
    if(NOT CMAKE_MATCH_1)
      message(
        FATAL_ERROR
...
set(_em_pkgconfig_libdir "${EMSCRIPTEN_SYSROOT}/local/lib/pkgconfig"
...
if("${CMAKE_VERSION}" VERSION_LESS "3.20")
  ...
  cmake_path(CONVERT "${_em_pkgconfig_libdir}" TO_NATIVE_PATH_LIST
...
set(EMSCRIPTEN
    1
    CACHE BOOL "If true, we are targeting Emscripten output.")
...
function(em_add_tracked_link_flag target flagname)
  ...
  foreach(jsFileList ${ARGN})
    foreach(jsfile ${jsFileList})
      ...
      # build target on that source file, and make the project depend on that
      # target.
      ...
      add_custom_command(
        OUTPUT ${dummy_c_name}
        COMMAND ${CMAKE_COMMAND} -E touch ${dummy_c_name}
        DEPENDS ${jsfile})
      target_link_libraries(${target} ${dummy_lib_name})

      # Link the js-library to the target When a linked library starts with a
      # "-" cmake will just add it to the linker command line as it is.  The
      # advantage of doing it this way is that the js-library will also be
      # automatically linked to targets that depend on this target.
      get_filename_component(js_file_absolute_path "${jsfile}" ABSOLUTE)
      target_link_libraries(${target}
                            "${flagname} \"${js_file_absolute_path}\"")
...
# This function links a (list of ) .js library file(s) to the given CMake
# project. Example: em_link_js_library(my_executable "lib1.js" "lib2.js") will
# result in emcc passing --js-library lib1.js --js-library lib2.js to the
# emscripten linker, as well as tracking the modification timestamp between the
# linked .js files and the main project, so that editing the .js file will cause
# the target project to be relinked.
...
if("${CMAKE_GENERATOR}" MATCHES "^Visual Studio.*")
  ...
  # <GenerateManifest>true</GenerateManifest> directive. This causes VS to
  # attempt to invoke rc.exe during the build, which will fail since app
  # manifests are meaningless for Emscripten.  To disable this, add the
  # following linker flag. This flag will not go to emcc, since the Visual
  ...
  # instead of a file, which causes emcc autogenerate the output filename. CMake
  # is hardcoded to assume all object files have the suffix .obj, so adjust the
  # emcc-autogenerated default suffix name to match.
  set(EMSCRIPTEN_VS_LINKER_FLAGS
      "${EMSCRIPTEN_VS_LINKER_FLAGS} --default-obj-ext .obj")
  # Also hint CMake that it should not hardcode <ObjectFileName> generation.
  # Requires a custom CMake build for this to work (ignored on others) See
  # http://www.cmake.org/Bug/view.php?id=14673 and https://github.com/juj/CMake
  set(CMAKE_VS_NO_DEFAULT_OBJECTFILENAME 1)

  # Apply and cache Emscripten Visual Studio IDE-specific linker flags.
  set(CMAKE_EXE_LINKER_FLAGS
      "${CMAKE_EXE_LINKER_FLAGS} ${EMSCRIPTEN_VS_LINKER_FLAGS}"
      CACHE STRING "")
  set(CMAKE_SHARED_LINKER_FLAGS
      "${CMAKE_SHARED_LINKER_FLAGS} ${EMSCRIPTEN_VS_LINKER_FLAGS}"
      CACHE STRING "")
...
# TODO: CMake appends <sysroot>/usr/include to implicit includes; switching to
# use usr/include will make this redundant.
