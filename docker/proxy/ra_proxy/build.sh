...
case "$PLATFORM" in
  sim)
    IMAGE_TAG=${DOCKER_REG}/trustflow-ra-proxy-sim-ubuntu22.04:${VERSION}
    LATEST_TAG=${DOCKER_REG}/trustflow-ra-proxy-sim-ubuntu22.04:latest
    echo -e "Building ${GREEN}${IMAGE_TAG}${NO_COLOR}"
    cd ../../.. && docker build . -f docker/proxy/ra_proxy/Dockerfile -t ${IMAGE_TAG} \
      --build-arg TEE_PLAT="sim" 
    ;;
  tdx)
    IMAGE_TAG=${DOCKER_REG}/trustflow-ra-proxy-tdx-ubuntu22.04:${VERSION}
    LATEST_TAG=${DOCKER_REG}/trustflow-ra-proxy-tdx-ubuntu22.04:latest
    echo -e "Building ${GREEN}${IMAGE_TAG}${NO_COLOR}"
    cd ../../.. && docker build . -f docker/proxy/ra_proxy/Dockerfile -t ${IMAGE_TAG} \
      --build-arg TEE_PLAT="tdx"
    ;;
  csv)
    IMAGE_TAG=${DOCKER_REG}/trustflow-ra-proxy-csv-ubuntu22.04:${VERSION}
    LATEST_TAG=${DOCKER_REG}/trustflow-ra-proxy-csv-ubuntu22.04:latest
    echo -e "Building ${GREEN}${IMAGE_TAG}${NO_COLOR}"
    cd ../../.. && docker build . -f docker/proxy/ra_proxy/Dockerfile -t ${IMAGE_TAG} \
