...
#include "trustedflow/attestation/verification/interface/verifier.h"

namespace trustedflow {
namespace attestation {
namespace verification {

class Sgx2AttestationVerifier : public AttestationVerifier {
 public:
  explicit Sgx2AttestationVerifier(const std::string& report_json_str)
      : AttestationVerifier(report_json_str) {
    Init();
  }

  explicit Sgx2AttestationVerifier(
  ...
  static std::unique_ptr<AttestationVerifier> Create(
      const secretflowapis::v2::sdc::UnifiedAttestationReport& report) {
    return std::make_unique<Sgx2AttestationVerifier>(report);
  }
